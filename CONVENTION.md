...
- 앱 전체의 600px 모바일 프레임은 `AppComposition`의 전역 shell이 담당한다.
  ...
  다시 만들지 않는다.
- 일반 사용자 화면은 393px Figma 좌표계를 통째 확대하지 않고 `w-full`, `px`,
  `flex/grid`, `aspect-ratio` 같은 컨테이너 기준 레이아웃으로 600px 프레임까지
  반응한다.
- Figma 수치를 옮길 때도 `left`, `top`, 고정 `width`를 그대로 복사하기보다
  의미 있는 컨테이너, 간격, 정렬, 최대 폭으로 다시 표현한다. 원본 폭은 참고값으로
  기록하되 런타임 레이아웃 기준으로 고정하지 않는다.
- 이미지와 영상은 용도에 맞는 `aspect-*` 비율을 먼저 정하고, 그 비율 안에서
  `object-cover` 또는 `object-contain`으로 처리한다. crop이 발생하는 미디어는
  `object-position`을 명시해 넓은 화면에서도 중요한 영역이 예측 가능하게 남도록
  한다.
- 읽어야 하는 텍스트가 이미지 위에 올라가는 경우, 이미지 색상에 가독성을 맡기지
  않는다. contrast overlay, gradient scrim, text shadow, 별도 safe area 중 하나로
  최소 대비를 보장한다.
- 히어로처럼 이미지 위에 텍스트, 로고, 장식 요소가 함께 올라가는 화면은 전경 요소를
  같은 visual stage나 앱 프레임 중앙 기준에 묶는다. 한 요소는 viewport 왼쪽에,
  다른 요소는 중앙에 고정하는 식으로 기준을 섞지 않는다.
- 미디어 히어로의 기본 레이어 순서는 media, contrast overlay, 보조 전경 텍스트,
  주요 로고·장식, 네비게이션·인터랙션 컨트롤 순으로 쌓는다.
- 한 장의 이미지 안에 있는 CD, 인물, 제품 같은 전경 오브젝트 뒤로 텍스트가
  지나가야 하면 z-index만 조정하지 않는다. 배경 이미지와 전경 오브젝트 레이어를
  분리하거나, 원본에서 투명 전경 레이어를 만들어 같은 stage에 겹친다.
- 좌표 보존과 scale 계산은 축제 지도처럼 지도 좌표와 화면 좌표를 맞춰야 하는
  화면에만 허용한다. 지도는 컨테이너 폭을 기준으로 마커 좌표와 배율을 계산하고,
  일반 UI에는 통째 `zoom`이나 scale wrapper를 쓰지 않는다.
- 상단 고정 네비게이션은 전역 앱 프레임 폭에 맞춰 `w-full max-w-[600px]` 기준으로
  배치하고, 본문에는 64px spacer를 둔다.
