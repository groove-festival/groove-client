...
find "$TARGET_DIR" -type f -name "*.js" -exec sed -i \
  ...
  -e "s|__VITE_TELEMETRY_ENABLED__|$VITE_TELEMETRY_ENABLED|g" \
  -e "s|__VITE_CLARITY_PROJECT_ID__|$VITE_CLARITY_PROJECT_ID|g" \
  -e "s|__VITE_SENTRY_DSN__|$VITE_SENTRY_DSN|g" \
  -e "s|__SENTRY_AUTH_TOKEN__|$SENTRY_AUTH_TOKEN|g" \
  -e "s|__SENTRY_ORG__|$SENTRY_ORG|g" \
  -e "s|__SENTRY_PROJECT__|$SENTRY_PROJECT|g" \
  {} +
EOF
